...
void columpiar(Canario& canario) {
    bool esHembra = canario.sexo[0] == 'H';
...
int main() {
    ...
    Canario c("Juan", new char('M'));
    Canario c1("Andrea", new char('H'));
    Canario c2("Samantha", new char('H'));
    Canario c3("Andres", new char('M'));
    Canario c4("Joan", new char('M'));
    threads[0] = thread(hiloCanario, ref(c));//ref retorna la direccion de memoria del objeto que le estamos pasando, protege esa seccion de memoria, lo usamos por que puede que cuando le pasemos un canario
    //a un hilo, su direccion de memoria haya cambiado
